...
EOF
    ...
    items+=('system_maintenance|DNS 和 Spotlight 检查|刷新 DNS 缓存并验证 Spotlight 状态|true')
    items+=('cache_refresh|Finder 缓存刷新|刷新 QuickLook 缩略图和图标服务缓存|true')
    items+=('saved_state_cleanup|应用状态清理|移除旧的保存状态（30 天以上）|true')
    ...
    items+=('sqlite_vacuum|数据库优化|压缩邮件、Safari 和信息的 SQLite 数据库（运行中的应用会跳过）|true')
    items+=('launch_services_rebuild|LaunchServices 修复|修复"打开方式"菜单和文件关联|true')
    items+=('font_cache_rebuild|字体缓存重建|重建字体数据库以修复渲染问题（浏览器运行中会跳过）|true')
    items+=('dock_refresh|Dock 刷新|修复 Dock 中的图标异常和显示问题|true')
    items+=('prevent_network_dsstore|防止 Finder .DS_Store|设置 Finder 偏好，停止在 SMB/AFP/NFS 和 USB 卷上写入 .DS_Store|true')

    # System performance optimizations (auto-run, non-destructive)
    items+=('memory_pressure_relief|内存优化|释放非活跃内存以提高系统响应速度|true')
    items+=('network_stack_optimize|网络栈刷新|清除路由表和 ARP 缓存以解决网络问题|true')
    items+=('disk_permissions_repair|权限修复|修复用户目录权限问题|true')
    items+=('spotlight_index_optimize|Spotlight 优化|搜索缓慢时重建索引（智能检测）|true')
    items+=('periodic_maintenance|定期维护|运行 macOS 每日/每周/每月维护脚本（如已过期）|true')
    items+=('shared_file_list_repair|共享文件列表|修复损坏的 Finder 收藏夹和最近文稿|true')
    items+=('disk_verify|磁盘健康|验证文件系统完整性|true')
    items+=('login_items_audit|登录项|审查登录项中的失效条目|true')

    # System database cleanup (auto-run, low risk)
    items+=('quarantine_cleanup|隔离数据库清理|清除 Gatekeeper 下载跟踪记录|true')
    items+=('launch_agents_cleanup|启动代理清理|移除程序已不存在的失效 LaunchAgent|true')
    items+=('notification_cleanup|通知清理|清除旧的已送达通知以减少数据库膨胀|true')
    items+=('coreduet_cleanup|使用数据|清除旧的使用跟踪数据|true')
