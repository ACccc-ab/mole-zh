...
_batch_scan_app_details() {
    ...
    if [[ ${#blocked_apps[@]} -gt 0 ]]; then
        ...
        for blocked_detail in "${blocked_apps[@]}"; do
            ...
            log_warning "$blocked_name 需要使用 $blocked_vendor 官方卸载器"
        done
    fi
}
...
_batch_preview_and_confirm() {
    ...
    if [[ "$has_brew_cask" == "true" ]]; then
        echo -e "${GRAY}${ICON_WARNING} Homebrew 应用将被完全清理，--zap 会移除配置和数据${NC}"
    fi
    ...
    for detail in "${app_details[@]}"; do
        ...
        while IFS= read -r file; do
            if [[ -n "$file" && -e "$file" ]]; then
                echo -e "  ${BLUE}${ICON_WARNING}${NC} 系统: $file"
            fi
        done <<< "$system_files"

        while IFS= read -r file; do
            if [[ -n "$file" && -e "$file" ]]; then
                echo -e "  ${YELLOW}${ICON_WARNING}${NC} 仅供参考: $file"
            fi
    ...
    local app_text="应用"
    [[ $app_total -gt 1 ]] && app_text="应用"

    echo ""
    local removal_note="移除 ${app_total} 个${app_text}"
    [[ -n "$size_display" ]] && removal_note+=", ${size_display}"
    if [[ ${#running_apps[@]} -gt 0 ]]; then
        removal_note+=" ${YELLOW}[运行中]${NC}"
    fi
    echo -ne "${PURPLE}${ICON_ARROW}${NC} ${removal_note}  ${GREEN}Enter${NC} 确认，${GRAY}ESC${NC} 取消: "
    ...
    if [[ "${MOLE_DRY_RUN:-0}" != "1" ]] &&
        ...
        if [[ ${#sudo_apps[@]} -gt 0 && ${#brew_cask_apps[@]} -eq 0 ]]; then
            admin_prompt="系统应用需要管理员权限：${sudo_apps[*]}"
        elif [[ ${#brew_cask_apps[@]} -gt 0 && ${#sudo_apps[@]} -eq 0 ]]; then
            admin_prompt="Homebrew cask 需要管理员权限：${brew_cask_apps[*]}"
        fi
        ...
        if [[ -z "$reason" ]]; then
            ...
            if [[ ${#leftover_paths[@]} -gt 0 ]]; then
                for _lpath in "${leftover_paths[@]}"; do
                    echo -e "  ${YELLOW}${ICON_WARNING}${NC} 无法移除：${_lpath/$HOME/~}"
                done
...
_batch_render_summary() {
    ...
    if [[ $success_count -gt 0 ]]; then
        local success_text="应用"
        [[ $success_count -gt 1 ]] && success_text="应用"
        local success_line="已移除 ${success_count} 个${success_text}"
        if is_uninstall_dry_run; then
            success_line="将移除 ${success_count} 个${success_text}"
        fi
    ...
    if [[ ${#local_network_warning_apps[@]} -gt 0 ]]; then
        ...
        summary_details+=("${ICON_REVIEW} macOS 15+ 的本地网络权限可能在应用移除后残留：${YELLOW}${local_network_list}${NC}")
        summary_details+=("${GRAY}${ICON_SUBLIST}${NC} Mole 不会重置 ${GRAY}/Volumes/Data/Library/Preferences/com.apple.networkextension*.plist${NC}")
        summary_details+=("${GRAY}${ICON_SUBLIST}${NC} 如果残留或重复条目仍然存在，请在恢复模式中手动清除，因为重置是全局性的${NC}")
    fi
    ...
    if [[ ${#system_extension_warning_apps[@]} -gt 0 ]]; then
        ...
        summary_details+=("${ICON_REVIEW} 系统扩展可能在移除后残留：${YELLOW}${ext_list}${NC}")
        summary_details+=("${GRAY}${ICON_SUBLIST}${NC} 请在 ${GRAY}系统设置 > 通用 > 登录项与扩展${NC} 中移除残留扩展")
    fi
    ...
    if [[ ${#background_items_warning_apps[@]} -gt 0 ]]; then
        ...
        summary_details+=("${ICON_REVIEW} 后台项目仍已注册：${YELLOW}${bg_list}${NC}")
        summary_details+=("${GRAY}${ICON_SUBLIST}${NC} 打开 ${GRAY}系统设置 > 通用 > 登录项与扩展${NC} 并关闭相应条目以清除")
    fi
    ...
    if [[ ${#running_at_uninstall_apps[@]} -gt 0 ]]; then
        ...
        summary_details+=("${ICON_REVIEW} 卸载时仍在运行，文件已移除但进程仍存活：${YELLOW}${running_list}${NC}")
        summary_details+=("${GRAY}${ICON_SUBLIST}${NC} 退出应用以释放其内存副本；在退出前重新安装可能会导致异常")
    fi
...
batch_uninstall_applications() {
    ...
    if [[ $success_count -gt 0 && ${#success_dock_targets[@]} -gt 0 ]]; then
        if is_uninstall_dry_run; then
            log_info "[预览模式] 将刷新 LaunchServices 并更新 Dock 条目"
        else
            (
