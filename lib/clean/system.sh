...
clean_deep_system() {
    ...
    stop_section_spinner
    [[ $cache_cleaned -eq 1 ]] && log_success "系统缓存"
    ...
    stop_section_spinner
    [[ $third_party_logs_cleaned -eq 1 ]] && log_success "第三方系统日志"
    ...
    if [[ $rebuildable_cache_cleaned -gt 0 ]]; then
        ...
        [[ $rebuildable_cache_cleaned -eq 1 ]] && rebuildable_cache_label="项"
        log_success "可重建的系统缓存，$rebuildable_cache_cleaned $rebuildable_cache_label"
    fi

    start_section_spinner "正在扫描可重建的 GPU 缓存..."
    ...
    if [[ $gpu_cache_cleaned -gt 0 ]]; then
        ...
        [[ $gpu_cache_cleaned -eq 1 ]] && gpu_cache_label="项"
        log_success "可重建的 GPU 缓存，$gpu_cache_cleaned $gpu_cache_label"
    fi
    ...
    if sudo test -d "$mem_reports_dir" 2> /dev/null; then
        ...
        if [[ "$file_count" -gt 0 ]]; then
            if [[ "${DRY_RUN:-}" != "true" ]]; then
                ...
                log_info "[预览模式] 将移除 $file_count 个旧内存异常报告（$total_size_kb KB）"
            fi
        fi
    fi
    stop_section_spinner
