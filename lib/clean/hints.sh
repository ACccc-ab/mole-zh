...
show_system_data_hint_notice() {
    ...
    echo -e "  ${GRAY}${ICON_REVIEW}${NC} 检查：mo analyze、设备备份、docker system df"
}
...
show_project_artifact_hint_notice() {
    ...
    if [[ $PROJECT_ARTIFACT_HINT_ESTIMATE_SAMPLES -gt 0 ]]; then
        ...
        if [[ "$estimate_is_partial" == "true" ]]; then
            echo -e "  ${GREEN}${ICON_LIST}${NC} ${GREEN}${hint_count_label}${NC} 个候选项，至少 ${estimate_human}（从 ${PROJECT_ARTIFACT_HINT_ESTIMATE_SAMPLES} 项中采样）"
        else
            echo -e "  ${GREEN}${ICON_LIST}${NC} ${GREEN}${hint_count_label}${NC} 个候选项，采样 ${estimate_human}"
        fi
    else
        echo -e "  ${GREEN}${ICON_LIST}${NC} ${GREEN}${hint_count_label}${NC} 个候选项"
    fi

    if [[ -n "$example_text" ]]; then
        echo -e "  ${GRAY}${ICON_SUBLIST}${NC} 示例：${GRAY}${example_text}${NC}"
    fi
    ...
    echo -e "  ${GRAY}${ICON_REVIEW}${NC} 检查：${review_command}"
}
...
show_user_launch_agent_hint_notice() {
    ...
    echo -e "  ${GRAY}${ICON_REVIEW}${NC} 检查：打开 ~/Library/LaunchAgents 并仅移除您认识的项目"
}
