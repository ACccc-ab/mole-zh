...
check_tcc_permissions() {
    ...
    if [[ "$needs_permission_check" == "true" ]]; then
        ...
        echo ""
        echo -ne "${PURPLE}${ICON_ARROW}${NC} 按 ${GREEN}Enter${NC} 继续： "
        read -r
    ...
    while IFS=$'\t' read -r record_root cache_dir; do
        ...
        case "${cache_dir##*/}" in
            ".next")
                ...
                current_python_root=""
                current_python_dirs=()
                [[ -d "$cache_dir/cache" ]] && clean_project_cache_target "$cache_dir/cache"/* "Next.js 构建缓存" || true
                ;;
            "__pycache__")
            ...
            ".dart_tool")
                ...
                current_python_dirs=()
                if [[ -d "$cache_dir" ]]; then
                    clean_project_cache_target "$cache_dir" "Flutter 构建缓存（.dart_tool）" || true
                    local build_dir="$(dirname "$cache_dir")/build"
                    if [[ -d "$build_dir" ]]; then
                        clean_project_cache_target "$build_dir" "Flutter 构建缓存（build/）" || true
                    fi
                fi
                ;;
        esac
...
clean_python_bytecode_cache_group() {
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        ...
        if [[ $skipped_count -gt 0 ]]; then
            echo -e "  ${YELLOW}${ICON_DRY_RUN}${NC} Python 字节码缓存 · ${display_root}${NC}, ${YELLOW}${removed_count} dirs, $(colorize_human_size "$size_human") ${YELLOW}dry, ${skipped_count} skipped${NC}"
        else
            echo -e "  ${YELLOW}${ICON_DRY_RUN}${NC} Python 字节码缓存 · ${display_root}${NC}, ${YELLOW}${removed_count} dirs, $(colorize_human_size "$size_human") ${YELLOW}dry${NC}"
        fi
    else
        local line_color
        line_color=$(cleanup_result_color_kb "$total_size_kb")
        if [[ $skipped_count -gt 0 ]]; then
            echo -e "  ${line_color}${ICON_SUCCESS}${NC} Python 字节码缓存 · ${display_root}${NC}, ${line_color}${removed_count} dirs, ${size_human}${NC}, ${skipped_count} skipped"
        else
            echo -e "  ${line_color}${ICON_SUCCESS}${NC} Python 字节码缓存 · ${display_root}${NC}, ${line_color}${removed_count} dirs, ${size_human}${NC}"
        fi
    fi
