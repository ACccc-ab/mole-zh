...
clean_trash() {
    ...
    if [[ -t 1 ]]; then
        MOLE_SPINNER_PREFIX="  " start_inline_spinner "正在清空回收站，${trash_count} 项..."
    fi
...
clean_user_essentials() {
    ...
    safe_clean ~/Library/Caches/* "用户应用缓存"
    stop_section_spinner

    safe_clean ~/Library/Logs/* "用户应用日志"

    start_section_spinner "正在清理运行时文件..."
    ...
    for target_path in "${mail_dirs[@]}"; do
        if [[ -d "$target_path" ]]; then
            if [[ "$spinner_active" == "false" && -t 1 ]]; then
                start_section_spinner "正在清理旧邮件附件..."
                spinner_active=true
            fi
            local dir_size_kb=0
...
clean_app_caches() {
    ...
    safe_clean ~/Library/Caches/GeoServices/* "地图地理瓦片缓存"
...
clean_browsers() {
    ...
    if [[ "$_chrome_running" != "true" ]]; then
        ...
        safe_clean ~/Library/Application\ Support/Google/Chrome/component_crx_cache/* "Chrome 组件 CRX 缓存"
    ...
    if [[ "$firefox_running" == "true" ]]; then
        ...
        safe_clean ~/Library/Caches/Firefox/* "Firefox 缓存"
    fi
    ...
    if [[ "$firefox_running" == "true" ]]; then
        ...
        safe_clean ~/Library/Application\ Support/Firefox/Profiles/*/cache2/* "Firefox 配置文件缓存"
    fi
    clean_chrome_old_versions
    clean_edge_old_versions
    clean_edge_updater_old_versions
...
clean_cloud_storage() {
    ...
    if pgrep -x "Dropbox" > /dev/null 2>&1; then
        ...
        safe_clean ~/Library/Caches/com.dropbox.* "Dropbox 缓存"
        safe_clean ~/Library/Caches/com.getdropbox.dropbox "Dropbox 缓存"
    fi
    if pgrep -x "Google Drive" > /dev/null 2>&1; then
        echo -e "  ${GRAY}${ICON_WARNING}${NC} Google Drive is running · cache cleanup skipped"
    else
        safe_clean ~/Library/Caches/com.google.GoogleDrive "Google Drive 缓存"
    fi
    ...
    if pgrep -x "OneDrive" > /dev/null 2>&1; then
        ...
        safe_clean ~/Library/Caches/com.microsoft.OneDrive "OneDrive 缓存"
    fi
}
        ...
        for candidate in "${gc_candidates[@]}"; do
            if [[ -d "$candidate" ]]; then
                ...
                if [[ "$quick_count" -gt 100 ]]; then
                    ...
                    stop_section_spinner
                    start_section_spinner "正在扫描 Application Support... 组 [$container_label, 批量清理]"
                    if [[ "$DRY_RUN" != "true" ]]; then
                        safe_remove "$candidate" true > /dev/null 2>&1 || true
                    fi
                    found_any=true
                ...
                while IFS= read -r -d '' item; do
                    ...
                    if ((candidate_item_count % 250 == 0)); then
                        ...
                        if [[ "$current_time" =~ ^[0-9]+$ ]] && ((current_time - last_progress_update >= 1)); then
                            ...
                            stop_section_spinner
                            start_section_spinner "正在扫描 Application Support... 组 [$container_label, $candidate_item_count 项]"
                            last_progress_update=$current_time
                        fi
                    fi
                    if [[ "$DRY_RUN" != "true" ]]; then
...
check_large_file_candidates() {
    ...
    _report_large_review_dir() {
        ...
        echo -e "  ${YELLOW}${ICON_WARNING}${NC} ${label}：${GREEN}${size_human}${NC}${GRAY}，路径：$path${NC}"
        found_any=true
    }
    ...
    if [[ "${SYSTEM_CLEAN:-false}" != "true" ]] && command -v tmutil > /dev/null 2>&1 &&
        ...
        if [[ -n "$snapshot_list" ]]; then
            ...
            if [[ "$snapshot_count" =~ ^[0-9]+$ && "$snapshot_count" -gt 0 ]]; then
                ...
                echo -e "  ${GRAY}${ICON_REVIEW}${NC} ${GRAY}检查：tmutil listlocalsnapshots /${NC}"
                found_any=true
            fi
        fi
    fi
    ...
    if command -v docker > /dev/null 2>&1; then
        ...
        if [[ -n "$docker_output" ]]; then
            ...
            if [[ -n "$docker_output" ]]; then
                echo -e "  ${YELLOW}${ICON_WARNING}${NC} Docker storage:"
                echo -e "  ${GRAY}${ICON_REVIEW}${NC} ${GRAY}运行：docker system df${NC}"
                found_any=true
            fi
        fi
    fi
