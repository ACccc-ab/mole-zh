...
paginated_multi_select() {
    ...
    draw_menu() {
        ...
        if [[ $visible_total -eq 0 ]]; then
            printf "${clear_line}没有可用的项目\n" >&2
            for ((i = 0; i < items_per_page; i++)); do
                printf "${clear_line}\n" >&2
            done
            printf "${clear_line}${GRAY}${ICON_NAV_UP}${ICON_NAV_DOWN}  |  Space  |  Enter 保存  |  Q 取消${NC}\n" >&2
            printf "${clear_line}" >&2
            return
        fi
        ...
        if [[ -n "$filter_text" ]]; then
            local -a _segs_filter=("${GRAY}Backspace${NC}" "${GRAY}Ctrl+U 清除${NC}" "${GRAY}ESC 清除${NC}")
