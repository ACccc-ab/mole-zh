...
opt_diag_family_note() {
    case "$1" in
        cloudshell)
            printf '%s\n' "检测到外部企业代理压力。Mole 不会终止企业安全进程；重启或策略检查需在 Mole 外部进行。"
            ;;
        syspolicyd)
            printf '%s\n' "Gatekeeper 和代码签名评估活动频繁。"
            ;;
        windowserver)
            printf '%s\n' "桌面合成繁忙。当其他类别占用更高时，应视其为症状而非根因。"
            ;;
        spotlight)
            printf '%s\n' "元数据索引或导入工作正在消耗 CPU。"
            ;;
...
opt_diag_offer_detach_candidates() {
    ...
    echo -e "    ${GRAY}${ICON_SUBLIST} 保留其中一个挂载：通过以下命令添加路径 ${NC}mo optimize --whitelist${GRAY}${NC}"
