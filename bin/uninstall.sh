...
_scan_resolve_uncached() {
    ...
    update_scan_status "正在扫描应用程序..." "0" "$total_apps"
    ...
    if ((total_apps > 0)); then
        for app_data_tuple in "${app_data_tuples[@]}"; do
            ...
            pids+=($!)
            update_scan_status "正在扫描应用程序..." "$app_count" "$total_apps"
...
match_apps_by_name() {
    ...
    for search_term in "${search_terms[@]}"; do
        ...
        if [[ "$found" == "false" ]]; then
            echo -e "${YELLOW}警告:${NC} 找不到匹配 '$search_term' 的应用程序"
        fi
    done
}
...
main() {
    ...
    if [[ ${#app_name_args[@]} -gt 0 ]]; then
        ...
        echo -e "${BLUE}${ICON_CONFIRM}${NC} 匹配了 ${selection_count} 个应用:"
        local index=1
