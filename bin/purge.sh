...
perform_purge() {
    ...
    if [[ $total_size_cleaned -gt 0 ]]; then
        ...
        if [[ "${MOLE_DRY_RUN:-0}" == "1" ]]; then
            summary_line="可释放空间: ${GREEN}${freed_size_human}${NC}"
        fi
        [[ $total_items_cleaned -gt 0 ]] && summary_line+=" | 项目: $total_items_cleaned"
        summary_line+=" | 可用: $(get_free_space)"
        summary_details+=("$summary_line")
    else
        summary_details+=("没有旧的项目产物需要清理。")
        summary_details+=("可用空间: $(get_free_space)")
    fi
...
show_help() {
    ...
    echo "  --dry-run       预览清理操作，不做任何更改"
    echo "  --include-empty 显示零大小的项目产物目录"
    echo "  --debug         启用调试日志"
    echo "  --help          显示此帮助信息"
    echo ""
