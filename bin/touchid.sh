...
enable_touchid() {
    ...
    if touchid_dry_run_enabled; then
        if is_touchid_configured; then
            ...
            echo -e "${GREEN}${ICON_SUCCESS} [预览] 将为 sudo 启用 Touch ID${NC}"
        ...
        if [[ -f "$PAM_SUDO_LOCAL_FILE" ]] && grep -q "pam_tid.so" "$PAM_SUDO_LOCAL_FILE"; then
            ...
            if grep -q "pam_tid.so" "$PAM_SUDO_FILE"; then
                ...
                if secure_install_pam "$temp_file" "$PAM_SUDO_FILE" 2> /dev/null; then
                    echo -e "${GREEN}${ICON_SUCCESS} 已清理旧配置${NC}"
                fi
            fi
...
disable_touchid() {
    ...
    if touchid_dry_run_enabled; then
        if ! is_touchid_configured; then
            ...
            echo -e "${GREEN}${ICON_SUCCESS} [预览] 将为 sudo 禁用 Touch ID${NC}"
            echo -e "${GRAY}${ICON_REVIEW} Target files: ${PAM_SUDO_FILE} and/or ${PAM_SUDO_LOCAL_FILE}${NC}"
        fi
        return 0
    fi
...
show_menu() {
    ...
    show_status
    if is_touchid_configured; then
        echo -ne "${PURPLE}☛${NC} 按 ${GREEN}Enter${NC} 禁用，${GRAY}Q${NC} 退出: "
        ...
        echo -ne "${PURPLE}☛${NC} 按 ${GREEN}Enter${NC} 启用，${GRAY}Q${NC} 退出: "
        IFS= read -r -s -n1 key || key=""
        drain_pending_input # Clean up any escape sequence remnants
...
main() {
    ...
    if touchid_dry_run_enabled; then
        echo -e "${YELLOW}${ICON_DRY_RUN} 预览模式${NC}，不会修改 sudo 认证文件"
        echo ""
    fi
