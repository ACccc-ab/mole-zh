...
select_installers() {
    ...
    draw_menu() {
        ...
        printf "${PURPLE_BOLD}选择要删除的安装包${NC}%s ${GRAY}, ${selected_human}, 已选 ${selected_count} 个${NC}\n" "$scroll_indicator"
        printf "%s\n" "$clear_line"
    ...
    if [[ $total_deleted -gt 0 ]]; then
        ...
        if [[ "$dry_run_mode" == "1" ]]; then
            ...
            if [[ $total_delete_failed -eq 0 ]]; then
                summary_details+=("你的 Mac 更整洁了！")
            fi
        fi
    else
        summary_details+=("没有安装包被删除")
    fi

    if [[ $total_delete_failed -gt 0 ]]; then
        local failure_label="个安装包"
        [[ $total_delete_failed -eq 1 ]] && failure_label="个安装包"
        summary_details+=("删除失败 ${YELLOW}$total_delete_failed${NC} $failure_label")
