...
if [[ -f "$HOME/.config/mole/whitelist" ]]; then
    while IFS= read -r line; do
        ...
        line="${line//\$\{HOME\}/$HOME}"
        if [[ "$line" =~ \.\. ]]; then
            WHITELIST_WARNINGS+=("不允许路径遍历: $line")
            continue
        fi

        if [[ "$line" != "$FINDER_METADATA_SENTINEL" ]]; then
            if [[ "$line" =~ [[:cntrl:]] ]]; then
                WHITELIST_WARNINGS+=("无效路径格式: $line")
                continue
            fi
...
end_section() {
    ...
    if [[ "${TRACK_SECTION:-0}" == "1" && "${SECTION_ACTIVITY:-0}" == "0" ]]; then
        echo -e "  ${GREEN}${ICON_SUCCESS}${NC} 没有需要清理的内容"
    fi
    TRACK_SECTION=0
}
...
EOF
    ...
    if [[ -t 0 ]]; then
        if has_sudo_session; then
            ...
            echo -ne "${PURPLE}${ICON_ARROW}${NC} 系统缓存需要 sudo。${GREEN}Enter${NC} 继续，${GRAY}Space${NC} 跳过: "
            ...
            if [[ "$choice" == "SPACE" ]]; then
                ...
                if ensure_sudo_session "系统清理需要管理员权限"; then
                    ...
                    SYSTEM_CLEAN=false
                    echo ""
                    echo -e "${YELLOW}认证失败${NC}，继续用户级别清理"
                fi
            else
                SYSTEM_CLEAN=false
                echo -e " ${GRAY}已跳过${NC}"
        ...
        if has_sudo_session; then
            SYSTEM_CLEAN=true
            echo "  ${ICON_LIST} 系统级清理已启用，sudo 会话活跃"
        else
            SYSTEM_CLEAN=false
            echo "  ${ICON_LIST} 系统级清理已跳过，需要 sudo"
        fi
        echo "  ${ICON_LIST} 用户级清理将自动执行"
        echo ""
    fi
}
...
perform_cleanup() {
    ...
    if [[ -z "$EXTERNAL_VOLUME_TARGET" && "${MOLE_TEST_MODE:-0}" == "1" ]]; then
        ...
        if [[ ${#WHITELIST_PATTERNS[@]} -gt 0 ]]; then
            ...
            echo -e "${GREEN}${ICON_SUCCESS}${NC} 已发现受保护项目"
        fi
        if [[ "$DRY_RUN" == "true" ]]; then
            echo ""
    ...
    if [[ ${#WHITELIST_PATTERNS[@]} -gt 0 ]]; then
        ...
        if [[ $custom_count -gt 0 || $predefined_count -gt 0 ]]; then
            local summary=""
            [[ $predefined_count -gt 0 ]] && summary+="$predefined_count 内置"
            [[ $custom_count -gt 0 && $predefined_count -gt 0 ]] && summary+=" + "
            [[ $custom_count -gt 0 ]] && summary+="$custom_count 自定义"
            summary+=" 个模式生效"
    ...
    if [[ -t 1 && "$DRY_RUN" != "true" ]]; then
        ...
        if [[ $fda_status -eq 1 ]]; then
            echo ""
            echo -e "${GRAY}${ICON_REVIEW}${NC} ${GRAY}在系统设置中授予终端完全磁盘访问权限以获得最佳效果${NC}"
        fi
    fi
    ...
    if [[ -n "$EXTERNAL_VOLUME_TARGET" ]]; then
        ...
        if run_with_shell_timeout 300 run_cloud_and_office_cleanup; then
            ...
            local ret=$?
            if [[ $ret -eq 124 ]]; then
                log_warning "Cloud & Office 清理超时（超过 5 分钟），跳过剩余项目"
            elif [[ $ret -eq 130 ]]; then
                return 130
            else
                log_warning "Cloud & Office 清理失败，退出码 $ret"
            fi
        fi
        end_section
    ...
    if [[ $total_size_cleaned -gt 0 ]]; then
        ...
        if [[ "$DRY_RUN" == "true" ]]; then
            local stats="可回收空间: $(colorize_human_size "$freed_size_human")"
        ...
        summary_status="info"
        if [[ "$DRY_RUN" == "true" ]]; then
            summary_details+=("未检测到明显可回收空间，系统已很整洁。")
        else
            summary_details+=("系统已很整洁，无额外空间可释放。")
        fi
