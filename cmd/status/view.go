...
func renderProcessAlertBar(alerts []ProcessAlert, width int) string {
	...
	if len(active) > 1 {
		text += fmt.Sprintf(" · 另有%d项", len(active)-1)
	}
...
func renderCPUCard(cpu CPUStatus, thermal ThermalStatus) cardData {
	...
	if cpu.PerCoreEstimated {
		lines = append(lines, subtleStyle.Render("单核数据不可用，使用平均负载估算"))
	...
	if cpu.PCoreCount > 0 && cpu.ECoreCount > 0 {
		...
		lines = append(lines, fmt.Sprintf("负载   %.2f / %.2f / %.2f, %d核",
...
func renderMemoryCard(mem MemoryStatus, cardWidth int) cardData {
	...
	if hasSwap {
		...
		lines = append(lines, fmt.Sprintf("余量   %s", humanBytes(mem.Available)))
	} else {
		// Layout without Swap:
		// 3. Total
		// 4. Cached (if > 0)
...
func renderDiskCard(disks []DiskStatus, io DiskIOStatus, trashSize uint64, trashApprox bool) cardData {
	var lines []string
	if len(disks) == 0 {
		lines = append(lines, subtleStyle.Render("采集中..."))
	} else {
	...
	if trashSize > 0 {
		...
		lines = append(lines, fmt.Sprintf("%-6s %s%s", "回收站", prefix, humanBytesShort(trashSize)))
	}
...
func renderNetworkCard(netStats []NetworkStatus, history NetworkHistory, proxy ProxyStatus, cardWidth int) cardData {
	...
	if len(netStats) == 0 {
		lines = []string{subtleStyle.Render("采集中...")}
	} else {
...
func renderBatteryCard(batts []BatteryStatus, thermal ThermalStatus) cardData {
	...
	if len(batts) == 0 {
		...
		if thermal.AdapterPower > 0 && isPoweredByAC(statusLower) {
			statusText += fmt.Sprintf(" · %.0fW 适配器", thermal.AdapterPower)
		}
		...
		if b.CycleCount > 0 {
			cycleText := fmt.Sprintf("%d 循环", b.CycleCount)
		...
		if thermal.BatteryTemp > 0 {
			tempText := "电池 " + colorizeTemp(thermal.BatteryTemp) + "°C"
	...
	return cardData{icon: iconBattery, title: "电池", lines: lines}
}
