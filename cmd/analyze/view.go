...
func (m model) View() string {
	...
	if m.inOverviewMode() {
		...
		if m.overviewScanning {
			if allOverviewEntriesPending(m.entries) {
				...
				fmt.Fprintf(&b, "%s%s%s%s 正在分析磁盘用量...\n\n",
			...
			if hasPendingOverviewEntries(m.entries) {
				...
				fmt.Fprintf(&b, "%s选择一个位置进行探索：%s\n\n", colorGray, colorReset)
			}
		}
	} else {
		fmt.Fprintf(&b, "%s磁盘分析%s  %s%s%s", colorPurpleBold, colorReset, colorGray, displayPath(m.path), colorReset)
		if !m.scanning {
	...
	if m.deleting {
		...
		fmt.Fprintf(&b, "%s%s%s%s 正在删除：%s%s 个项目%s已移除，请稍候...\n",
			colorCyan, colorBold,
	...
	if m.scanning {
		...
		fmt.Fprintf(&b, "%s%s%s%s 正在扫描%s：%s%s 个文件%s，%s%s 个目录%s，%s%s%s\n",
			colorCyan, colorBold,
			spinnerFrames[m.spinner],
			colorReset,
			progressPrefix,
	...
	if m.inOverviewMode() {
		...
		if selectCount > 0 {
			fmt.Fprintf(&b, "%s↑↓← | 空格 选择 | R 刷新 | O 打开 | P 预览 | F 文件 | ⌫ 删除 %d | Esc 返回 | Q/Ctrl+C 退出%s\n", colorGray, selectCount, colorReset)
		} else {
			fmt.Fprintf(&b, "%s↑↓← | 空格 选择 | R 刷新 | O 打开 | P 预览 | F 文件 | ⌫ 删除 | Esc 返回 | Q/Ctrl+C 退出%s\n", colorGray, colorReset)
		}
	} else {
		...
		if selectCount > 0 {
			if largeFileCount > 0 {
				fmt.Fprintf(&b, "%s↑↓←→ | 空格 选择 | 回车 | R 刷新 | O 打开 | P 预览 | F 文件 | ⌫ 删除 %d | T 大文件 %d | Esc 返回 | Q/Ctrl+C 退出%s\n", colorGray, selectCount, largeFileCount, colorReset)
			} else {
				fmt.Fprintf(&b, "%s↑↓←→ | 空格 选择 | 回车 | R 刷新 | O 打开 | P 预览 | F 文件 | ⌫ 删除 %d | Esc 返回 | Q/Ctrl+C 退出%s\n", colorGray, selectCount, colorReset)
			}
		} else {
			if largeFileCount > 0 {
				fmt.Fprintf(&b, "%s↑↓←→ | 空格 选择 | 回车 | R 刷新 | O 打开 | P 预览 | F 文件 | ⌫ 删除 | T 大文件 %d | Esc 返回 | Q/Ctrl+C 退出%s\n", colorGray, largeFileCount, colorReset)
			} else {
				fmt.Fprintf(&b, "%s↑↓←→ | 空格 选择 | 回车 | R 刷新 | O 打开 | P 预览 | F 文件 | ⌫ 删除 | Esc 返回 | Q/Ctrl+C 退出%s\n", colorGray, colorReset)
			}
		}
	}
	if m.deleteConfirm && m.deleteTarget != nil {
		...
		if deleteCount > 1 {
			fmt.Fprintf(&b, "%s删除：%s %d 个项目，%s  %s按回车确认  |  ESC 取消%s\n",
				colorRed, colorReset,
