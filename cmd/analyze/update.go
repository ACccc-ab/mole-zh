...
func (m *model) scheduleOverviewScans() tea.Cmd {
	...
	if len(pendingIndices) > 0 {
		...
		if len(pendingIndices) == 1 {
			...
			m.status = fmt.Sprintf("正在扫描 %d 个目录...，剩余 %d", len(pendingIndices), remaining)
		}
	}
	...
	case tickMsg:
		...
		if m.scanning || m.deleting || (m.inOverviewMode() && (m.overviewScanning || hasPending)) {
			...
			if m.deleting && m.deleteCount != nil {
				...
				if count > 0 {
					m.status = fmt.Sprintf("正在移到废纸篓... %s 个项目", formatNumber(count))
				}
			}
			return m, tickCmd()
		}
			...
			if len(m.largeFiles) > 0 {
				if len(m.largeMultiSelected) > 0 {
					...
					if count > maxBatchOpen {
						m.status = fmt.Sprintf("打开项目过多，最多 %d，已选 %d", maxBatchOpen, count)
						return m, nil
					}
					for path := range m.largeMultiSelected {
						go func(p string) {
			...
			if len(m.largeFiles) > 0 {
				if len(m.largeMultiSelected) > 0 {
					...
					if count > maxBatchReveal {
						m.status = fmt.Sprintf("显示项目过多，最多 %d，已选 %d", maxBatchReveal, count)
						return m, nil
					}
					for path := range m.largeMultiSelected {
						go func(p string) {
		...
		if m.showLargeFiles {
			if len(m.largeFiles) > 0 && m.largeSelected < len(m.largeFiles) {
				...
				if count > 0 {
					...
					m.status = fmt.Sprintf("已选 %d 项，%s", count, humanizeBytes(totalSize))
				} else {
					m.status = fmt.Sprintf("已扫描 %s", humanizeBytes(m.totalSize))
				}
			}
